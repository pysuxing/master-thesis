...
施加在向量流上。
地\ref{sec:code-generation}节介绍如何从程序的流表示生成C代码。
...
使用这组精简的原语集足够构建更多更为复杂的向量操作，FIXME：提及第三章中的向量操作举例说明。
...
一种和\texttt{map}类似的操作，他们区别在于\texttt{map}只允许
将标量操作施加到一维向量之上，而\texttt{foreach}指令则无这种限制。
...
系统的任务调度器会负责调度任务执行。Rat编译期生成的C程序框架如下：
...
向量流图中的每一条边都被打包成一个\texttt{task}类型的结构，
然后通过运行时系统API装入任务调度器。这种封装方式保存了向量流图
的逻辑结构，运行时系统将依靠向量流图实现任务的动态调度。
